...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save admin user data to Firestore (adminusers collection)
      await setDoc(doc(db, 'adminusers', user.uid), {
        ...
        phone: '',
        phoneNumber: '',
        active: true, // Admin users are active by default
        status: 'Active',
  ...
  return (
    <div className='flex w-screen h-screen overflow-hidden bg-white'>
      {/* Background patterns - removed gradient */}
      
      {/* Left side signup form - reversed from login page */}
      <div className='w-full md:w-1/2 flex items-center justify-center p-4 order-2 md:order-1 relative bg-gradient-to-br from-slate-50 via-white to-blue-50/30'>
        ...
        <div className='w-full max-w-md relative z-10 px-4'>
          <Card className='card-premium shadow-2xl border-0 bg-gradient-to-br from-white via-white to-blue-50/30 relative overflow-hidden rounded-2xl'>
            ...
            <CardHeader className='space-y-4 pb-4 relative z-10 pt-8'>
              ...
              <CardTitle className='text-3xl font-bold text-center bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 bg-clip-text text-transparent'>Create an Account</CardTitle>
              <CardDescription className='text-center text-sm text-gray-600'>Start managing your institution today</CardDescription>
            </CardHeader>
              ...
              <form onSubmit={handleSignup} className='space-y-4'>
                ...
                <div className='flex items-start space-x-3 pt-1'>
                  ...
                  <label htmlFor='terms' className='text-xs sm:text-sm text-gray-600 leading-relaxed'>
                    I agree to the <Link href='#' className='text-primary hover:underline font-semibold transition-colors'>Terms</Link> and <Link href='#' className='text-primary hover:underline font-semibold transition-colors'>Privacy Policy</Link>
                  </label>
                </div>
      ...
      <div className='hidden md:flex w-1/2 bg-primary items-center justify-center relative order-1 md:order-2 overflow-hidden'>
        ...
        <div className='absolute top-0 left-0 w-full h-full'>
          ...
          {/* Small glowing orb */}
          <div className='absolute top-[22%] left-[55%] w-3 h-3 rounded-full bg-blue-200/20 shadow-lg shadow-blue-200/30 blur-[2px]'></div>
        ...
        <div className='max-w-lg text-center z-10 px-8'>
          <h2 className='text-3xl md:text-4xl font-bold text-white mb-4'>Start your educational journey</h2>
          <p className='text-blue-100 text-lg mb-8'>A comprehensive solution for managing college applications, admissions, and student data.</p>
          
          <div className='backdrop-blur-sm bg-white/10 p-5 rounded-xl border border-white/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)]'>
            <p className='text-white font-medium italic'>"The beautiful thing about learning is that no one can take it away from you."</p>
            <p className='mt-2 text-sm text-blue-200'>— B.B. King</p>
          </div>
        </div>
